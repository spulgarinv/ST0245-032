...
public class MiArrayList <T> {
     ...
     */
    public MiArrayList() {
        elements = new int[this.DEFAULT_CAPACITY];
        size = 0;
    }
    ...
    public void add(int e) {
        if (elements.length - size <= elements.length / 2) {
            newsize();
        }
        elements[size++] = e;
    }
    ...
    public void add(int index, int e) {
        if (index < size ){ //
            ...
            for (int temp = index; index < size - 1; temp++ ){
             int temp2 = temp + 1;
             elements[temp] = e;
             elements[temp2] = elements[temp];
    ...
    public void del(int index){
        ...
        if (index < size ){
            int temp = 0;
            for(temp = index; temp < size - 1; temp++) {
